...
import { savePosts, getPosts, savePost } from '@/utils/localStorage';
...
export function Dashboard() {
  ...
  // Set page title
  useEffect(() => {
    setPageTitle(pageSEO.dashboard.title);
  }, []);
  ...
  const handlePost = async () => {
    ...
    try {
      ...
      if (data) {
        await savePost(data.id, data);
        
        // Update posts list in local storage
        const cachedPosts = await getPosts(user.id) || [];
        await savePosts(user.id, [data, ...cachedPosts]);
      }
